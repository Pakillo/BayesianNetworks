...
stancode <- function(plant_effort = c("param", "data"),
  ...
  scode_data <- paste0(
    ...
    "  real<lower=0> beta;\n",
    switch(plant_abun,
           param = "  real<lower=0> alpha_sigma;\n",
           data = "  vector<lower=0>[n_p] abun_p;\n"),
    switch(animal_abun,
           param = "  real<lower=0> alpha_tau;\n",
  ...
  scode_tdata <- paste0(
    "transformed data {\n",
    ifelse(plant_abun == "param" | animal_abun == "param",
           "// Prior parameters for abundances\n", ""),
    switch(plant_abun,
           param = "  vector<lower=0>[n_p] a_sigma = rep_vector(alpha_sigma, n_p);\n",
           data = ""),
    switch(animal_abun,
           param = "  vector<lower=0>[n_a] a_tau = rep_vector(alpha_tau, n_a);\n",
           data = ""),
  ...
  scode_model <- paste0(
    ...
    switch(animal_pref,
           ...
           varying = paste0("  for (j in 1:n_a) {\n",
                            ...
                            "  }\n")),
    switch(plant_abun,
           param = "  sigma ~ dirichlet(a_sigma);\n",
           data = ""),
    switch(animal_abun,
           param = "  tau ~ dirichlet(a_tau);\n",
           data = ""),
